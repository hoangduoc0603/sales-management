...
Mỗi yêu cầu tạo/hoàn tất/duyệt/đảo phải mang `commandId` và khóa idempotency do client tạo. `CommandTransaction` có trạng thái tối thiểu `Preparing`, `Committed`, `Failed`; chỉ dữ liệu của command `Committed` được tính vào số dư, báo cáo và kết quả nghiệp vụ. Sau timeout, client phải tra cứu `commandId` hoặc gửi lại cùng idempotency key; backend trả kết quả commit cũ hoặc kết quả recovery, không tạo nhóm chứng từ/ledger thứ hai.
