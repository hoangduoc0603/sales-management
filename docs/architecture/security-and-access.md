...
- Session token là opaque random secret; phiên thường browser chỉ giữ trong `sessionStorage`/memory, phiên “ghi nhớ đăng nhập” được lưu trong `localStorage` khi người dùng chủ động tick checkbox trên thiết bị cá nhân. Server chỉ lưu HMAC fingerprint theo tenant session pepper, user ID, issued time, idle expiry, absolute expiry, auth version và revoke status. Production Apps Script phải sinh token/record ID bằng UUID/random generator; ID deterministic chỉ được dùng trong test/dev composition để có fixture ổn định.
- Session thường idle tối đa 1 giờ và absolute lifetime 8 giờ. Session “ghi nhớ đăng nhập” opt-in có idle và absolute lifetime tối đa 7 ngày. Bất cứ reset password, disable user hoặc thay đổi role/scope nào đều tăng auth version và revoke session hiện có.
