...
LLD không thay đổi các quyết định đã Accepted về public Web App, modular monolith, storage role/partition, POS performance, command journal, customer-owned deployment, backup/restore hoặc actor metadata theo ADR 0017. Mọi thay đổi các quyết định này phải có ADR mới và chỉ có hiệu lực sau khi được phê duyệt.
...
| Side effect | Document, ledger, projection quyết định kết quả và actor metadata trên record phải xong trước success. Export, PDF, backup và notification dùng outbox/worker khi cần; baseline không lưu audit nghiệp vụ riêng. |
...
| User/role/scope, import/export, báo cáo và vận hành | [LLD Administration–Reporting–Operations](modules/administration-reporting-operations.md) và [Operations–Reporting tables](../data-model/tables/operations-reporting.md) |
...
6. Side effect/outbox/worker, failure/retry/recovery và actor metadata.
