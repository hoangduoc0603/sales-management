...
| `deposit-credit-refund-cancel-order` | 8 | `BaselineCovered` | Nhận cọc trên đơn online trước `Shipped` phải xử lý được khi hủy. | `tests/apps-script/sales/sales-service.test.ts`, `tests/shared/sales-contracts.test.ts` và `tests/web/local-fake-backend.test.ts` cover giữ cọc thành `CustomerCredit` hoặc refund counter-payment. |
...
| `operations-private-attachment-drive` | 11 | `BaselineCovered` | File đính kèm thuộc phạm vi chứng từ/chi phí/bảo hành; không được public URL. | `operations.attachment.upload/list/delete/download` dùng private Drive storage seam, kiểm quyền/scope, logical delete và không trả public URL. Tích hợp UI/source object thuộc scope từng domain. |
