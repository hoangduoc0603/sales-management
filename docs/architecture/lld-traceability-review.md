...
| --- | --- | --- | --- |
| `SRS-OVR` | [Detailed Design](detailed-design.md), [Platform](platform-technical-design.md), Administration–Operations | [Registry](../data-model/sheet-schema-and-registry.md), operations tables; ADR 0001–0010 | Tenant/scope, public app/session nội bộ, command lock/idempotency, worker, lifecycle, audit/backup/archive/performance. |
...
| `SRS-ACC` | [Administration–Reporting–Operations](modules/administration-reporting-operations.md) | [Operations–Reporting tables](../data-model/tables/operations-reporting.md) | User/role/scope, audit, import/export/attachment, dashboard/report, backup/restore/health. |
...
| Command outcome | Mutation có `commandId`, fresh-read trong lock ngắn, `CommandTransaction` và `AuditOutbox`; retry tra cứu kết quả cũ, không tạo duplicate. |
...
| Performance | POS dùng cache local cho đọc, server-authoritative checkout; report/export/import/backup/PDF/audit delivery ở worker hoặc ngoài fast path. Query route active/archive partition, không quét toàn bộ history. |
